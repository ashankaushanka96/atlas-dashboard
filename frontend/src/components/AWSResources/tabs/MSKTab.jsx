...
const getMskStateConfig = (state) => {
  const normalized = String(state || "").toLowerCase();
  if (normalized === "active") return { color: "#34D399", Icon: CheckCircleIcon };
  if (["creating", "updating"].includes(normalized)) {
...
const MSKTab = ({ data, globalSearch, globalSearchResults, onRefresh }) => {
  const ADVANCED_SEARCH_PREFIX = "msk";
  ...
  const { localSearch, updateLocalSearch, clearLocalSearch } = useLocalSearch('msk', globalSearch);
  ...
  const uniqueStatuses = useMemo(() => {
    const statuses = [...new Set(data.clusters.map(cluster => cluster.state))].filter(Boolean).sort();
    return statuses;
  }, [data.clusters]);
  ...
  const advancedFields = useMemo(
    () => [
      ...
      { value: "kafka_version", label: "Kafka Version", type: "text" },
      { value: "number_of_broker_nodes", label: "Broker Count", type: "number" },
    ],
  ...
  const {
    ...
    reorderColumns,
    resetToDefault,
  } = useTableColumns("aws_msk", MSK_DEFAULT_COLUMNS);
  ...
  const filteredClusters = useMemo(() => {
    ...
    if (searchResults) {
      ...
      clusters = clusters.filter(cluster => 
        ...
        cluster.state?.toLowerCase().includes(term) ||
        cluster.region?.toLowerCase().includes(term) ||
        cluster.kafka_version?.toLowerCase().includes(term)
      );
    }
    ...
    // Apply status filter
    if (selectedStatus) {
      clusters = clusters.filter(cluster => cluster.state === selectedStatus);
    }
  ...
  const sortedClusters = useMemo(() => {
    const getComparableValue = (cluster, column) => {
      switch (column) {
        ...
        case "kafka_version":
          return cluster.kafka_version || "";
        case "cluster_arn":
          return cluster.cluster_arn || "";
        case "number_of_broker_nodes":
          return Number(cluster.number_of_broker_nodes || 0);
        default:
          return "";
      }
    };
  ...
  const columnDefs = {
    ...
    state: {
      ...
      renderCell: (cluster) => {
        const stateConfig = getMskStateConfig(cluster.state);
        return (
          <TableCell key="state">
            <StatusChip label={cluster.state || "Unknown"} color={stateConfig.color} Icon={stateConfig.Icon} />
          </TableCell>
        );
      },
    },
    ...
    kafka_version: {
      renderHeader: () => sortableHeader("kafka_version", "Kafka Version"),
      renderCell: (cluster) => (
        <TableCell key="kafka_version">
          <StatusChip
            label={cluster.kafka_version || "N/A"}
            color="#A78BFA"
    ...
    number_of_broker_nodes: {
      renderHeader: () => sortableHeader("number_of_broker_nodes", "Broker Count"),
      renderCell: (cluster) => (
        <TableCell key="number_of_broker_nodes">
          <StatusChip label={String(cluster.number_of_broker_nodes || 0)} color="#60A5FA" Icon={DnsIcon} />
        </TableCell>
      ),
    },
    cluster_arn: {
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
      ...
      <ModalFactory
        ...
        type="msk"
        title={selectedCluster?.cluster_name || 'MSK Cluster Details'}
      />
    </Box>
  );
};
